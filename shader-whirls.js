// Original Shader Whirls - Integrated from shader folder
// Self-contained p5.js shader module

(function(){
  const whirlCount = 12;

  // Original vertex shader from vert.js
  const vert = `
  ...
  // Original fragment functions from frag_functions.js
  ...
  // Original fragment shader from frag.js
  const frag = `
    #ifdef GL_ES
    precision highp float;
    #endif
  ...
  // P5 instance mode to avoid global conflicts
  let p5Instance = null;
  let p5Canvas = null;

  const sketch = (p) => {
    let theShader, whirls = [], userControl = false;
  ...
  function ensureInstance(){
    ...
    let container = document.getElementById('p5-shader-container');
    if (!container){
      container = document.createElement('div');
      container.id = 'p5-shader-container';
