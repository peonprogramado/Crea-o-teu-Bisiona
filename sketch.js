...
function exportCanvas() {
  try {
    // Exportar directamente como JPG sin mostrar diálogo
    const isJPG = true;
    const isPNG = false;
    ...
    if (isJPG) {
      ...
      // Área de captura expandida con el padding
      const captureX = Math.max(0, (svgScreenX - effectPadding) * dpr);
      const captureY = Math.max(0, (svgScreenY - effectPadding) * dpr);
      const captureWidth = Math.min(canvas.width - captureX, (svgScreenWidth + effectPadding * 2) * dpr);
      const captureHeight = Math.min(canvas.height - captureY, (svgScreenHeight + effectPadding * 2) * dpr);

      // Recalcular escala considerando el área expandida
      const expandedWidth = captureWidth / dpr;
      const expandedHeight = captureHeight / dpr;
      const adjustedScale = Math.min(targetWidth / expandedWidth, targetHeight / expandedHeight);

      // Recalcular dimensiones y centrado con el área expandida
      const finalWidth = expandedWidth * adjustedScale;
      const finalHeight = expandedHeight * adjustedScale;
      const adjustedOffsetX = (canvasSize - finalWidth) / 2;
      const adjustedOffsetY = (availableHeight - finalHeight) / 2 + canvasSize * 0.05;

      // Dibujar el SVG escalado con área expandida para efectos, correctamente centrado
      exportCtx.drawImage(
        canvas,
        captureX, captureY, // Posición fuente expandida
        captureWidth, captureHeight, // Tamaño fuente expandido
        adjustedOffsetX, adjustedOffsetY, // Posición destino centrada
        finalWidth, finalHeight // Tamaño destino escalado
      );
...
async function startCanvasRecording() {
  try {
    ...
    // Cargar el logo bisiona.png para el video
    const bisionaLogo = new Image();
    bisionaLogo.src = './assets/bisiona.png';

    // Función para dibujar el canvas principal en el canvas de grabación
    function drawFrame() {
      // Limpiar canvas de grabación con el color de fondo actual
      const isDarkTheme = document.body.classList.contains('dark-theme');
      recordingCtx.fillStyle = isDarkTheme ? '#000000' : '#fafafa';
      recordingCtx.fillRect(0, 0, 1080, 1350);

      // Obtener la transformación actual del SVG en el canvas principal
      const { scale: svgScale, tx, ty } = getTransformToFit();
      const dpr = window.devicePixelRatio || 1;

      // Agregar padding extra para capturar efectos de contorno sin cortar
      const extraPadding = 100 * dpr; // Padding generoso para contornos

      // Calcular el área del SVG en el canvas principal (en píxeles del canvas)
      const svgScreenX = Math.max(0, (tx + viewBox.x * svgScale) * dpr - extraPadding);
      const svgScreenY = Math.max(0, (ty + viewBox.y * svgScale) * dpr - extraPadding);
      const svgScreenWidth = Math.min(canvas.width - svgScreenX, viewBox.w * svgScale * dpr + extraPadding * 2);
      const svgScreenHeight = Math.min(canvas.height - svgScreenY, viewBox.h * svgScale * dpr + extraPadding * 2);

      // Calcular escala para que el SVG ocupe el 70% del video (más pequeño)
      const targetWidth = 1080 * 0.70;
      const targetHeight = 1350 * 0.70;
      const recordingScale = Math.min(targetWidth / svgScreenWidth, targetHeight / svgScreenHeight);

      // Calcular dimensiones finales del SVG en el video
      const finalWidth = svgScreenWidth * recordingScale;
      const finalHeight = svgScreenHeight * recordingScale;

      // Centrar perfectamente en el canvas de grabación
      const offsetX = (1080 - finalWidth) / 2;
      const offsetY = (1350 - finalHeight) / 2;

      // Dibujar el área del SVG con padding extra, escalada y centrada
      recordingCtx.drawImage(
        canvas,
        svgScreenX, svgScreenY, svgScreenWidth, svgScreenHeight, // Área fuente (SVG + padding para contornos)
        offsetX, offsetY, finalWidth, finalHeight // Área destino (escalada y centrada)
      );

      // Dibujar el logo bisiona.png en la parte inferior derecha
      if (bisionaLogo.complete && bisionaLogo.naturalWidth > 0) {
        const logoHeight = 60; // Altura del logo en píxeles
        const logoWidth = (bisionaLogo.width / bisionaLogo.height) * logoHeight;
        const logoMargin = 30; // Margen desde los bordes
        const logoX = 1080 - logoWidth - logoMargin;
        const logoY = 1350 - logoHeight - logoMargin;
    ...
    // Animar el canvas de grabación continuamente
    function animate() {
      if (isRecording) {
        drawFrame();
    ...
    alert('Error al iniciar la grabación. Asegúrate de que tu navegador soporta grabación de video.');
    return false;
  }
}
  ...
  try {
    ...
    if (isRecording) {
      ...
      if (stopCanvasRecording()) {
        saveBtn.textContent = '⏹ Deteniendo...';
        saveBtn.style.color = '#ff9800';
      }
    } else {
...
function initMobileIndicator() {
  if (isMobileDevice()) {
    ...
    // Mostrar tooltip temporal - DESACTIVADO
    /*
